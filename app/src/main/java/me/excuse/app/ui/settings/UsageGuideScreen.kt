 ...
 *
 * 八节结构完全一样，唯一的区别是默认展开还是收起：
 * - 前两节是"读一遍就够"的入门，默认展开，打开这一页看到的内容量和全展开时一样。
 * - 后六节是"带着问题回来查"的，默认收起，标题就是索引。
 *
                ...
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    GuideSection(
                        ...
                        body = "· 理由可以重复。同一个 app 同一天里再写一遍旧理由，只会在输入框下面提示" +
                                ...
                                "· 离开监控 app 有 10 秒宽限期：10 秒内回去接着原来的计时；超过 10 秒再打开会重新拦截。\n" +
                                "· 从一个监控 app 直接切到另一个，没有宽限期 —— 这一次立刻结束，新 app 立刻弹窗。\n" +
                                "· 时间到的时候如果你已经离开那个 app 了，不会追着你弹窗，这一次直接记完结束。"
                    )
                    ...
                    GuideSection(
                        ...
                        body = "「今日」「本周」「本月」三个 tab 可以横滑切换。「本周」是最近 7 天、" +
                                ...
                                "方块深浅按绝对时长分五档（15 / 45 / 90 分钟为界），不是跟自己的最高值比，" +
                                "下面有「少 → 多」的图例。\n" +
                    ...
                    GuideSection(
                        ...
                        body = "两类东西：\n" +
                                ...
                                "另外还有你勾的监控名单和手动改过的分类。\n" +
                                "「设置 → 导出记录」会把上面全部导成一个 JSON 文件，" +
                                "包括那些不展示的弹窗结果。目前只能导出，还不能在应用内删除。"
                    )
